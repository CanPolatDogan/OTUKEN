...
public class HealthSystem : MonoBehaviour
{
    ...
    public float flashIntensity = 0.4f; // Kırmızılık oranı (0-1 arası, 0.4 = %40 kırmızı)
    ...
    private void Start()
    {
        ...
        // Renderer'ları kaydet (düşmanlar için)
        if (!isPlayer)
        {
            CacheRenderers();
        }
    ...
    private System.Collections.IEnumerator FlashRedCoroutine()
    {
        ...
        // Hafif kırmızı tint uygula
        for (int i = 0; i < allMaterials.Count; i++)
        {
            Material mat = allMaterials[i];
            Color originalColor = originalColors[i];

            // Orijinal renk ile kırmızı arasında karışım (Lerp)
    ...
    public void ShowHealthBar(bool show)
    {
        if (healthBarUI != null)
        {
            ...
            if (show)
            {
                ...
                // Bar açılırken can değerlerini de güncelle!
                UpdateHealthUI();
            }
        }
    }
    ...
    private void Die()
    {
        ...
        if (!isPlayer)
        {
            ...
            // Collider'ı kapat (düşman üzerinden geçilebilsin)
            ...
            // Animasyon oynadıktan sonra objeyi yok et
        ...
        else
        {
            ...
            // InputManager'ı kapat (tuş inputlarını kes)
            ...
            // Tüm düşmanlara oyuncunun öldüğünü bildir
            NotifyEnemiesOfPlayerDeath();

            // Collider'ı kapat (düşmanlar üzerinden geçebilsin)
            Collider col = GetComponent<Collider>();
            if (col != null)
            {
                col.enabled = false;
            ...
            // Hedef seçim sistemini devre dışı bırak
    ...
    private void NotifyEnemiesOfPlayerDeath()
    {
        ...
        Debug.Log($"{allEnemies.Length} düşmana oyuncunun öldüğü bildirildi.");
    }
